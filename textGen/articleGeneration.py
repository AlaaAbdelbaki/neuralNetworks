...
class GRU(nn.Module):
    ...
    def forward(self, input, hidden):
        input = self.encoder(input.view(1, -1))
        output, hidden = self.gru(input.view(1, 1, -1), hidden)
        output, hidden = self.decoder(output.view(1, -1))
        output = self
        decoder(output.view(1, -1))
        return output, hidden
...
def training(n_epochs, file):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if epoch % print_every == 0:
            print('[%s (%d %d%%) %.4f (%.4f)]' % (time_since(start),
                  epoch, epoch / n_epochs * 100, loss_avg / epoch, loss))
    ...
    try:
        while True:
            ...
            if len(input1) > 0:
                ...
                print(evaluate(decoder=decoder, prime_str=input1,
                      predict_len=length, temperature=0.8))
            else:
...
if __name__ == '__main__':
    ...
    #
    parser.add_argument("-d", "--trainingData", default="data/shakespeare.txt",
                        type=str, help="trainingData [path/to/the/data]")
    parser.add_argument("-te", "--trainEval", default='train',
                        type=str, help="trainEval [train, eval]")
    #
    ...
    parser.add_argument("--decoder", default='RNN',
                        type=str, help="decoder [RNN]", choices=['RNN', 'LSTM', 'GRU'])
    #
    ...
    if args.decoder == 'RNN':
        print("Using RNN")
        run_name = "rnnGeneration"
        decoder = RNN(n_characters, args.hidden_size,
                      n_characters, args.num_layers).to(device)
    elif args.decoder == 'LSTM':
        print("Using LSTM")
        run_name = "lstmGeneration"
        decoder = LSTM(n_characters, args.hidden_size,
                       n_characters, args.num_layers).to(device)

    else:
        print("Using GRU")
        run_name = "gruGeneration"
        decoder = GRU(n_characters, args.hidden_size,
                      n_characters, args.num_layers).to(device)
