...
def train(input_line_tensor, target_line_tensor):
    target_line_tensor.unsqueeze_(-1)
    hidden = decoder.init_hidden(
        input_line_tensor.size(0))  # Adjust batch size

    decoder.zero_grad()

    loss = 0

    for i in range(input_line_tensor.size(1)):  # Loop through sequence length
        output, hidden = decoder(
            input_line_tensor[:, i:i+1, :].to(device), hidden)
        l = criterion(output.to(device), target_line_tensor[:, i].to(device))
        loss += l
    ...
    return output, loss.item() / input_line_tensor.size(1)
...
class LSTM(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        # Take the output at the last time step (if input is sequential)
        output = lstm_out[:, -1, :]  # Get the last time step of the sequence
    ...
    def init_hidden(self, batch_size):
        ...
        return (
            torch.zeros(1, batch_size, self.hidden_size).to(device),  # h_0
            torch.zeros(1, batch_size, self.hidden_size).to(device)   # c_0
        )

    def init_hidden_random(self, batch_size):
        # Initialize random hidden state (h_0) and cell state (c_0) on the specified device
        return (
            torch.rand(1, batch_size, self.hidden_size).to(device),  # h_0
            torch.rand(1, batch_size, self.hidden_size).to(device)   # c_0
        )
...
def training(n_epochs, lines):
    ...
    loss_csv = open(f'loss_{start}.csv', 'w')
...
def sample(decoder, start_letters='ABC'):
    with torch.no_grad():  # no need to track history in sampling
        ...
        hidden = decoder.init_hidden_random(20)
...
if __name__ == '__main__':
    ...
    # RNN(n_characters, args.hidden_size, n_characters, args.num_layers).to(device)
