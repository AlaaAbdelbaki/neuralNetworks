...
class LSTM(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.output_layer = nn.Linear(hidden_size + output_size, output_size)
    ...
    def forward(self, input, hidden):
        ...
        # hidden: (h_0, c_0) where both are (1, batch_size, hidden_size)
        # Add sequence dimension (batch_size, seq_len=1, input_size)
        input = input.unsqueeze(1)

        # lstm_out: (batch_size, seq_len=1, hidden_size)
        lstm_out, hidden = self.lstm(input, hidden)
        # Remove sequence dimension (batch_size, hidden_size)
        lstm_out = lstm_out.squeeze(1)

        output = self.hidden_to_output(lstm_out)
        output_combined = torch.cat((lstm_out, output), dim=1)
        output = self.output_layer(output_combined)
        output = self.dropout(output)
        output = self.softmax(output)
    ...
    def init_hidden_random(self,):
        # Initialize hidden state (h_0, c_0) with random values
        return (torch.rand(1, self.hidden_size, self.hidden_size),
                torch.rand(1, self.hidden_size, self.hidden_size))


def training(n_epochs, lines, light: bool):
    print()
    print('-----------')
    print('|  TRAIN  |')
    print('-----------')
    ...
    start = time.time()
    loss_csv = open(
        f'losses/loss{math.floor(start)}{"_light" if light else ""}.csv', 'w')
    loss_csv.write('epoch,loss\n')
    all_losses = []
    total_loss = 0
    best_loss = 100
...
if __name__ == '__main__':
    ...
    #
    parser.add_argument("-d", "--trainingData", default="data/RussianTrain.txt", type=str,
    ...
    parser.add_argument('--light', action='store_true', help="light model")
    #
    ...
    decode = None
    if args.light != None and args.light:
    ...
    elif args.run == 'lstmGeneration':
        decoder = LSTM(n_letters, 128, n_letters).to(device)
    else:
    ...
    modelFile = args.run + "_" + \
        str(args.num_layers) + "_" + str(args.hidden_size) + \
        ("_light" if args.light else "") + ".pt"
    ...
    if args.trainEval == 'train':
        modelPath = f'models/{modelFile}'
        decoder.train()
        training(n_epochs, lineTraining, args.light)
